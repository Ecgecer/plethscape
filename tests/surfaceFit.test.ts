...
test("wearable fitting follows a curved head and curved neck rather than floating on fixed planes", () => {
  ...
  const bandGeometry = new THREE.LatheGeometry(
    [new THREE.Vector2(0.176, -0.014), new THREE.Vector2(0.176, 0.014)],
    96,
  );
  bandGeometry.scale(1, 1, 0.229 / 0.176);
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    const expected =
      Math.sqrt(1 - Math.pow((3.5 + y - 3.4) / 0.26, 2)) /
      Math.sqrt(
        Math.pow(Math.sin(angle) / 0.17, 2) +
          Math.pow(Math.cos(angle) / 0.22, 2),
      );
    const gap = Math.hypot(x, z) - expected;
    assert(gap > 0.0003 && gap < 0.004, `Headband clearance ${gap}`);
  }
