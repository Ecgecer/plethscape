...
test("lung presentation groups preserve all source triangles, winding and attributes", () => {
  ...
  assert.deepEqual(Array.from(g.index!.array.slice(6)), [0, 1, 2]);
  g.dispose();
});
test("non-indexed meshes work and geometry outside the lung region stays untouched", () => {
  const g = new THREE.BufferGeometry();
  g.setAttribute(
    "position",
    new THREE.Float32BufferAttribute(positions.slice(9), 3),
  );
