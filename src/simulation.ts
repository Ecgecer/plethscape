...
export interface Physiology {
  ...
  noise: number;
  activity: Activity;
  /** Immutable activity transitions preserve phase and replay across gait changes. */
...
export const SITES: AnatomicalSite[] = [
  ...
  },
  {
    ...
    name: "Temple",
    shortName: "Temple",
    location: "Temple · superficial tissue",
    description:
      "An illustrative reflectance sensor at one temple. This reuses the head-site teaching contour; it is not a calibrated temple PPG model. Local contact and skin circulation affect real recordings.",
    delayMs: 85,
    mode: "reflectance",
  },
  {
...
export function sampleAccelerometer(
  ...
  const t = safe(timeSec, 0);
  const { m, a, impact } = movementSignal(t, p.activity, p.motionHistory);
  const amplitude = 0.27 * m.walk + 0.68 * m.run;
  return {
    x:
