 ...
 */
export type SiteId = 'finger' | 'wrist' | 'ear' | 'forehead' | 'carotid' | 'upperarm' | 'toe';
...
export const DEFAULT_PHYSIOLOGY: Physiology = {
  age: 32, heartRate: 72, respiratoryRate: 16, stiffness: 35, perfusion: 72, noise: 9, activity: 'rest',
};

export const SITES: AnatomicalSite[] = [
  { id: 'finger', name: 'Index finger', shortName: 'Finger', location: 'Digital vascular bed',
    description: 'A smart ring reads reflected light from the finger’s local vascular bed. This teaching model illustrates pulse shape; it does not reproduce a commercial ring’s data or algorithms.',
    delayMs: 195, mode: 'reflectance' },
  { id: 'wrist', name: 'Top of wrist', shortName: 'Wrist', location: 'Dorsal wrist microvasculature',
    description: 'Reflectance PPG samples superficial blood-volume changes at the wrist. Its signal is especially sensitive to contact and arm movement.',
    delayMs: 155, mode: 'reflectance' },
  { id: 'ear', name: 'Earlobe', shortName: 'Ear', location: 'Auricular vascular bed',
    description: 'A transmission sensing site with an earlier pulse arrival than the finger in this schematic anatomy. PWDB uses a temporal-artery approximation for the ear.',
    delayMs: 80, mode: 'transmission' },
  { id: 'forehead', name: 'Forehead', shortName: 'Forehead', location: 'Forehead microvasculature',
    description: 'An illustrative reflectance site. Sensor geometry and skin circulation influence its optical waveform; this site is an extension beyond the supplied PWDB sites.',
    delayMs: 85, mode: 'reflectance' },
  { id: 'carotid', name: 'Carotid / neck', shortName: 'Carotid', location: 'Carotid region · model reference',
    description: 'A proximal pulse-volume reference for learning propagation. Neck optical measurements exist in research, but this trace is not a direct measurement of carotid pressure or flow.',
    delayMs: 45, mode: 'reference' },
  { id: 'upperarm', name: 'Upper arm', shortName: 'Upper arm', location: 'Brachial / arm microvasculature',
    description: 'Reflectance PPG at the upper arm is represented by a broader local pulse contour and a shorter propagation path than the wrist.',
    delayMs: 115, mode: 'reflectance' },
  { id: 'toe', name: 'Great toe', shortName: 'Toe', location: 'Distal foot vascular bed',
    description: 'A distal reflectance band with a longer illustrative arrival delay. The toe model extends beyond PWDB’s anterior-tibial ankle site.',
    delayMs: 285, mode: 'reflectance' },
];
...
const smoothstep = (v: number) => { const x = unit(v); return x * x * (3 - 2 * x); };
const gaussian = (t: number, mu: number, sigma: number) => Math.exp(-0.5 * ((t - mu) / sigma) ** 2);
...
function correlatedBeatNoise(index: number, seed: number): number {
  return 0.6 * beatNoise(index, seed) + 0.3 * beatNoise(index - 1, seed)
    + 0.1 * beatNoise(index - 2, seed);
}

const SITE_SHAPE: Record<SiteId, { width: number; reflection: number; gain: number; motion: number }> = {
  ...
  forehead: { width: 1.10, reflection: 0.56, gain: 0.85, motion: 0.48 },
  carotid: { width: 1.04, reflection: 0.82, gain: 0.98, motion: 0.6 },
  upperarm: { width: 1.13, reflection: 0.76, gain: 0.79, motion: 0.75 },
  toe: { width: 1.10, reflection: 1.02, gain: 0.88, motion: 1.15 },
};

interface Model {
  cycle: number; age: number; stiffness: number; perfusion: number; noise: number;
  activity: number; cadence: number; respiratoryHz: number; delay: number;
  peakTime: number; reflectionTime: number; reflectionWidth: number;
  reflectionAmplitude: number; notchTime: number; notchWidth: number;
  notchAmplitude: number; gain: number; motionSensitivity: number;
  lastVariation?: { index: number; gain: number; width: number; reflection: number };
}
...
const cache = new WeakMap<Physiology, { values: number[]; activity: Activity; sites: Partial<Record<SiteId, Model>> }>();

function model(p: Physiology, siteId: SiteId): Model {
  let entry = cache.get(p);
  if (!entry || entry.values[0] !== p.age || entry.values[1] !== p.heartRate ||
      entry.values[2] !== p.stiffness || entry.values[3] !== p.perfusion ||
      entry.values[4] !== p.noise || entry.values[5] !== p.respiratoryRate || entry.activity !== p.activity) {
    entry = { values: [p.age, p.heartRate, p.stiffness, p.perfusion, p.noise, p.respiratoryRate], activity: p.activity, sites: {} };
    cache.set(p, entry);
  }
  ...
  const stiffness = clamp(0.10 + age * 0.65 + (unit(safe(p.stiffness, 35) / 100) - 0.35) * 0.60, 0.02, 0.96);
  ...
  const cycle = 60 / hr;
  const peakTime = clamp((0.153 - (hr - 72) * 0.00031) * shape.width, 0.105, cycle * 0.34);
  ...
  const reflectionTime = Math.min(peakTime + 0.220 - stiffness * 0.140, cycle * 0.73);
  const reflectionWidth = Math.min((0.037 + stiffness * 0.027) * shape.width, cycle * 0.115);
  const notchTime = peakTime + (reflectionTime - peakTime) * 0.66;
  const m: Model = {
    cycle, age, stiffness, perfusion, noise, activity,
    cadence: p.activity === 'run' ? 2.65 : p.activity === 'walk' ? 1.8 : 0,
    respiratoryHz: clamp(safe(p.respiratoryRate, 16), 6, 36) / 60,
    delay: site.delayMs / (0.88 + stiffness * 0.64) / 1000,
    peakTime, reflectionTime, reflectionWidth,
    reflectionAmplitude: (0.38 - 0.24 * stiffness) * shape.reflection,
    notchTime,
    notchWidth: Math.min(0.017 + stiffness * 0.010, cycle * 0.065),
...
function beat(phase: number, m: Model, width = 1, reflectionScale = 1): number {
  // Zero endpoints let independent beats tile without an ECG-like reset spike.
  if (phase <= 0 || phase >= 1) return 0;
  const t = phase * m.cycle / width;
  ...
  const reflected = m.reflectionAmplitude * reflectionScale * gaussian(t, m.reflectionTime, m.reflectionWidth);
  const notch = m.notchAmplitude * reflectionScale * gaussian(t, m.notchTime, m.notchWidth);
  const onset = smoothstep(phase / 0.045);
  const end = 1 - smoothstep((phase - 0.79) / 0.21);
  return Math.max(0, 0.92 * forward + 0.14 * runoff + reflected - notch) * onset * end * m.gain;
}

/** One clean beat. Phase 0 and 1 are the same foot; gain preserves perfusion. */
export function sampleBeat(phase0to1: number, p: Physiology, siteId: SiteId): number {
...
export function samplePPG(timeSec: number, p: Physiology, siteId: SiteId): number {
  ...
  const nominalCycle = localTime / m.cycle;
  const timingStrength = (1 - m.age * 0.5) * (1 - m.activity * 0.65);
  // Bounded, smooth phase offsets keep the clock monotonic and the long-term
  // mean at the selected HR. This adds modest nonperiodic variability to the
  // existing breathing modulation; it is not a fitted HRV/autonomic model.
  const cycles = nominalCycle + timingStrength * (
    0.012 * respiration
    + 0.018 * smoothNoise(nominalCycle / 2.3, 137)
    + 0.010 * smoothNoise(nominalCycle / 7.7, 419));
  const index = Math.floor(cycles);
  // Most successive samples belong to one beat. Cache its small morphology
  // adjustments, while keeping their values independent of sampling order.
  if (m.lastVariation?.index !== index) m.lastVariation = {
    index,
    gain: 1 + 0.018 * correlatedBeatNoise(index, 271),
    width: 1 + 0.012 * correlatedBeatNoise(index, 811),
    reflection: 1 + 0.045 * correlatedBeatNoise(index, 1613),
  };
  const variation = m.lastVariation;
  const vascularDrift = smoothNoise(localTime * 0.075, 2027);
  const clean = beat(wrap(cycles), m, variation.width, variation.reflection)
    * variation.gain * (1 + 0.022 * respiration + 0.012 * vascularDrift);
  const drift = 0.014 * respiration + 0.004 * Math.sin(TAU * 0.047 * t)
    + 0.003 * vascularDrift;
  const sensor = m.noise * (0.042 * Math.sin(TAU * 13.17 * t + 0.4)
    + 0.021 * Math.sin(TAU * 23.71 * t + Math.sin(t * 0.83))
    + 0.014 * Math.sin(TAU * 37.31 * t + 1.7));
  const cadencePhase = TAU * m.cadence * t;
  const movement = m.activity * m.motionSensitivity * (0.08 + 0.10 * (1 - m.perfusion)) * (
    Math.sin(cadencePhase + 0.6) + 0.4 * Math.sin(cadencePhase * 2 + 1.2)
...
export function sampleAccelerometer(timeSec: number, p: Physiology): { x: number; y: number; z: number } {
  const t = safe(timeSec, 0);
  ...
  return {
    x: amplitude * (Math.sin(a) + 0.18 * Math.sin(2 * a + 0.6)) + 0.004 * Math.sin(t * 1.3),
    y: amplitude * 0.68 * Math.sin(a + 1.1) + 0.006 * Math.sin(t * 1.1 + 0.8),
    z: 1 + amplitude * (0.64 * Math.cos(2 * a) + 0.13 * Math.sin(3 * a + 0.4)) + 0.004 * Math.sin(t * 1.2),
  };
}

/** Model descriptors: all percentages are illustrative, not clinical indexes. */
export function getMetrics(p: Physiology, siteId: SiteId): { transitMs: number; reflectionIndex: number; notchProminence: number } {
  const m = model(p, siteId);
  ...
  for (let i = 0; i <= 256; i++) {
    ...
    if (points[i] > peak) { peak = points[i]; peakIndex = i; }
  }
  let prominence = 0;
  for (let i = peakIndex + 2; i < 210; i++) {
    if (points[i] < points[i - 1] && points[i] <= points[i + 1]) {
      ...
      prominence = Math.max(prominence, (rebound - points[i]) / Math.max(peak, 0.001));
    }
  }
  return {
...
export function getInsight(p: Physiology, siteId: SiteId): { title: string; body: string } {
  if (p.activity !== 'rest') return {
    title: p.activity === 'run' ? 'Movement joins the pulse' : 'Two rhythms, one sensor',
  ...
  if (p.age >= 58 || p.stiffness >= 70) return {
