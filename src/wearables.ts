...
export function createWearables(): {
  ...
  function add(
    ...
    if (site === "forehead") {
      const smoothContact = new TessellateModifier(0.003, 8).modify(geometry);
      geometry.dispose();
      geometry = smoothContact;
    }
  ...
  // A single temple capsule: rounded amber face, black perimeter, hidden optics.
  const amberFace = physical({
    color: "#ae741e",
    metalness: 0.72,
    roughness: 0.29,
    clearcoat: 0.48,
  ...
  add(
    "forehead",
    roundedSolid(0.028, 0.052, 0.0015, 0.014, 0.0003),
    rubber,
    "Temple adhesive contact",
    at(0, 0, -0.0032),
  );
  add(
    "forehead",
    roundedSolid(0.026, 0.05, 0.006, 0.013),
    blackMetal,
    "Black capsule perimeter",
    at(0, 0, 0.0004),
  );
  add(
    "forehead",
    roundedSolid(0.0225, 0.046, 0.003, 0.0112),
    amberFace,
    "Satin amber temple face",
    at(0, 0, 0.0043),
  );
  add(
    "forehead",
    roundedSolid(0.009, 0.017, 0.001, 0.004, 0.0002),
    opticalGlass,
    "Skin-facing temple optics",
    at(0, 0, -0.0038),
  );
  add(
    "forehead",
    new THREE.SphereGeometry(0.001, 12, 8),
    ledMaterials.forehead,
    "Concealed temple emitter",
    at(0, -0.004, -0.0043),
  );
