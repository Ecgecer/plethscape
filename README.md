...
- Adjust arterial stiffness, peripheral perfusion and breathing rate, then switch between the scrolling stream and a labeled cardiac cycle. Breathing rate changes the lungs and respiratory modulation in the live PPG; the clean single-beat view excludes those artifacts.
...
The neutral presentation skin is generated with `scripts/build_neutral_skin.py` from BodyParts3D FJ2810. It preserves the elbow pose, turns the palms posteriorly, reconstructs a smooth forearm envelope, and removes external reproductive contours. The generated `public/models/neutral-skin.glb` replaces the reference skin at runtime; the original source GLB and metadata remain unchanged.

Latest verification (seven devices, orange anatomical presentation, revised skin, facial treatment and luminous flow trails): build passed, 20 Node tests passed, and all 17 browser tests passed. Browser checks include physical picking for all seven devices and the mobile learning flow. No browser errors were observed. Earlier frame-time measurements above predate this visual pass.
